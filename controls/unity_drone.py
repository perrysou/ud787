...
class UnityDrone(Drone):
    ...
    def local_acceleration_target(self):
        return np.array(
            [self._target_acceleration_north, self._target_acceleration_east, self._target_acceleration_down])
    ...
    def _show_plots(self):
        self._horizontal_plot = self._v.line(self.all_horizontal_errors, X=self.all_times,
                                             opts=dict(title="Horizontal Error", xlabel="Time(s)", ylabel="Error (m)"))
        self._vertical_plot = self._v.line(self.all_vertical_errors, X=self.all_times,
                                           opts=dict(title="Vertical Error", xlabel="Time(s)", ylabel="Error (m)"))
